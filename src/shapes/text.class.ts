...
class TextBase extends FabricObject {
  ...
  _measureChar(_char, charStyle, previousChar, prevCharStyle) {
    ...
    var fontCache = Text.getFontCache(charStyle), fontDeclaration = this._getFontDeclaration(charStyle),
  ...
  _applyPatternGradientTransformText(filler) {
    var pCanvas = createCanvasElement(), pCtx,
  ...
  _getFontDeclaration(styleObject, forMeasuring) {
    ...
    return [
      ...
      (isLikelyNode ? style.fontWeight : style.fontStyle),
      (isLikelyNode ? style.fontStyle : style.fontWeight),
   ...
   * @memberOf Text
   */
  static DEFAULT_SVG_FONT_SIZE = DEFAULT_SVG_FONT_SIZE;
