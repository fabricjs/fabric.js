  ...
  implements UniqueTextProps
{
  ...
  calcTextHeight() {
    ...
    for (let i = 0, len = this._textLines.length; i < len; i++) {
      ...
      if (i === len - 1) {
        ...
        height += Number.isFinite(divided) ? divided : this.fontSize;
      } else {
        height += lineHeight;
      }
    }
  ...
  _renderTextCommon(
    ...
    for (let i = 0, len = this._textLines.length; i < len; i++) {
      ...
      const maxHeight = Number.isFinite(divided) ? divided : this.fontSize;
  ...
  _renderChars(
    ...
    const topAdjustment = Number.isFinite(divided) ? divided : 0;
    top -= topAdjustment;
    if (shortCut) {
