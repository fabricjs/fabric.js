...
export type EnlivenObjectOptions = Abortable & {
   ...
   */
  reviver?: <T extends BaseFabricObject | FabricObject | BaseFilter | TFiller>(
...
export const enlivenObjects = <
  T extends BaseFabricObject | FabricObject | BaseFilter | TFiller
>(
  objects: any[],
...
>(
  ...
  new Promise<R>((resolve, reject) => {
    ...
    const promises = Object.values(serializedObject).map((value: any) => {
      ...
      // clipPath or gradient
      if (value.type) {
        return enlivenObjects<FabricObject | TFiller>([value], { signal }).then(
          ([enlived]) => {
