...
export const projectStrokeOnPoints = (
  ...
  */
  ...
  const projectionsOpenPath = (
    ...
    const proj1 = applySkew(A.add(orthogonalVector)), 
  ...
  const projectionsMiter = (
    ...
    if (hypot(miterVector.x, miterVector.y) / s <= adjustedStrokeMiterLimit) {
      ...
      return projectionsBevel(A, B, C, bisector)
    }
  };
  ...
  points.forEach(function (p, index) {
    ...
    if (strokeLineJoin === 'miter') {
      coords.push(...projectionsMiter(A, B, C, bisector));
      return;
    }
    else if (strokeLineJoin === 'round' && skewX === 0 && skewY === 0) {
      coords.push(...projectionsRoundNoSkew(A, B, C, bisector));
      return;
    } 
    else if (strokeLineJoin === 'round'&& (skewX !== 0 || skewY !== 0)) {
      coords.push(...projectionsRoundWithSkew(A, B, C, bisector));
      return;
    }
    else {
      coords.push(...projectionsBevel(A, B, C, bisector));
      return;
    }
  });
