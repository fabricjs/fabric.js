...
export const projectStrokeOnPoints = (
  ...
  points.forEach(function (p, index) {
    ...
    if (openPath && (index === 0 || index === points.length - 1)) {
      ...
      coords.push(...projectionsOpenPathButt(index, A, B, C, strokeUniformScalar, scale, s, strokeUniform, skewX, skewY));
      return;
    }
    ...
    if (strokeLineJoin === 'miter') {
      coords.push(...projectionsMiter(A, B, C, bisector, strokeUniformScalar, scale, s, strokeUniform, skewX, skewY, strokeMiterLimit));
      return;
    }
    else if (strokeLineJoin === 'round' && skewX === 0 && skewY === 0) {
      coords.push(...projectionsRoundNoSkew(A, bisector, s, strokeUniformScalar));
      return;
    } 
    else if (strokeLineJoin === 'round'&& (skewX !== 0 || skewY !== 0)) {
      coords.push(...projectionsRoundWithSkew(A, B, C, bisector, strokeUniformScalar, scale, s, strokeUniform, skewX, skewY));
      return;
    }
    else {
      coords.push(...projectionsBevel(A, B, C, bisector, strokeUniformScalar, scale, s, strokeUniform, skewX, skewY));
      return;
    }
  });
...
const scaleHatVector = (hatVector: Point, scalar: number, strokeUniformScalar: Point): Point => {
...
const projectionsOpenPathButt = (
  ...
  const D = index === 0 ? C : B, 
    ...
    orthogonalVector = scaleHatVector(hatOrthogonalVector, s, strokeUniformScalar);
...
const projectionsBevel = (
  ...
  return [AB, AC].map(vector => {
    const hatOrthogonal = getOrthogonalUnitVector(vector), 
      ...
      orthogonal = scaleHatVector(hatOrthogonal, s * correctSide, strokeUniformScalar),
...
const projectionsMiter = (
  ...
  else { // when the miter-limit is reached, the stroke line join becomes of type bevel
    return projectionsBevel(A, B, C, bisector, strokeUniformScalar, scale, s, strokeUniform, skewX, skewY)
  }
};
...
const projectionsRoundWithSkew = (
  ...
  const bisectorVector = bisector.vector,
    AB = createVector(strokeUniform ? A.multiply(scale) : A, strokeUniform ? B.multiply(scale) : B), 
    AC = createVector(strokeUniform ? A.multiply(scale) : A, strokeUniform ? C.multiply(scale) : C);
