...
function setupGitConfig() {
  ...
  try {
    ...
    console.log(
      '✅ Configured union merge driver for automatic CHANGELOG.md conflict resolution',
    );

    // Verify the configuration
    const unionDriver = execGitCommand(
    ...
    console.log(
      '📋 The CHANGELOG.md file will now automatically merge without conflicts',
    );
    console.log(
      '   using the union merge strategy as defined in .gitattributes',
    );
  } catch (error) {
