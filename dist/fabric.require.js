...
(function() {
    ...
    var split = "split";
    var apply = "apply";
    var math = Math, mmax = math.max, mmin = math.min, abs = math.abs, pow = math.pow, PI = math.PI, round = math.round, toFloat = parseFloat, toInt = parseInt;
    ...
    function cacher(f, scope, postprocessor) {
        function newf() {
            ...
            if (cache.hasOwnProperty(args)) {
                for (var i = 0, ii = count.length; i < ii; i++) if (count[i] === args) {
    ...
    var pow = Math.pow, sqrt = Math.sqrt, min = Math.min, max = Math.max;
    abs = Math.abs;
    function getBoundsOfCurve(x0, y0, x1, y1, x2, y2, x3, y3) {
        var tvalues = [], bounds = [ new Array(6), new Array(6) ], a, b, c, t, t1, t2, b2ac, sqrtb2ac;
        for (var i = 0; i < 2; ++i) {
            if (i == 0) {
                b = 6 * x0 - 12 * x1 + 6 * x2;
    ...
    var getBoundsOfPath = function(path) {
        ...
        var minx = min.apply(Number.MAX_VALUE, xbounds), miny = min.apply(Number.MAX_VALUE, ybounds), maxx = max.apply(Number.MIN_VALUE, xbounds), maxy = max.apply(Number.MIN_VALUE, ybounds);
        ...
        for (i = 0, ilen = curve.length; i < ilen; i++) {
            ...
            if (s[0] == "M") {
                ...
                isC = true;
                if (startX < minx || startX > maxx || startY < miny || startY > maxy || s[1] < minx || s[1] > maxx || s[2] < miny || s[2] > maxy || s[3] < minx || s[3] > maxx || s[4] < miny || s[4] > maxy || s[5] < minx || s[5] > maxx || s[6] < miny || s[6] > maxy) {
...
(function(global) {
    ...
    fabric.Path = fabric.util.createClass(fabric.Object, {
        ...
        _render: function(ctx) {
            var current, previous = null, x = 0, y = 0, controlX = 0, controlY = 0, tempX, tempY, tempControlX, tempControlY, l = -this.width / 2, t = -this.height / 2, subStartX = 0, subStartY = 0, k, klen;
            for (var i = 0, len = this.path.length; i < len; ++i) {
                ...
                switch (current[0]) {
                  ...
                  case "a":
                    ...
                    for (k = 0, klen = segs.length; k < segs.length; k += 6) ctx.bezierCurveTo.apply(ctx, segs.slice(k, k + 6));
                    x += current[6];
                    y += current[7];
                    break;
