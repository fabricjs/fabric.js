...
(function(global) {
    ...
    fabric.parseSVGDocument = function() {
        ...
        return function(doc, callback, reviver) {
            ...
            var viewBoxAttr = doc.getAttribute("viewBox"), widthAttr = parseFloat(doc.getAttribute("width")), heightAttr = parseFloat(doc.getAttribute("height")), width = null, height = null, viewBoxWidth, viewBoxHeight, minX, minY;
...
(function() {
    ...
    fabric.StaticCanvas = fabric.util.createClass({
        ...
        absolutePan: function(point) {
            var wh = fabric.util.transformPoint(new fabric.Point(this.getWidth(), this.getHeight()), this.viewportTransform);
...
(function() {
    ...
    fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, {
        ...
        _searchPossibleTargets: function(e) {
            var target, pointer = this.getPointer(e, true);
...
(function() {
    ...
    fabric.util.object.extend(fabric.Object.prototype, {
        ...
        setCoords: function() {
            var strokeWidth = this.strokeWidth > 1 ? this.strokeWidth : 0, theta = degreesToRadians(this.angle), vpt = this.getViewportTransform();
            var f = function(p) {
            ...
            var padX = Math.cos(_angle + theta) * this.padding * Math.sqrt(2), padY = Math.sin(_angle + theta) * this.padding * Math.sqrt(2);
...
(function() {
    ...
    fabric.util.object.extend(fabric.Object.prototype, {
        ...
        drawBorders: function(ctx) {
            ...
            var vpt = this.getViewportTransform(), wh = fabric.util.transformPoint(new fabric.Point(this.getWidth(), this.getHeight()), vpt, true), sxy = fabric.util.transformPoint(new fabric.Point(scaleX, scaleY), vpt, true), w = wh.x, h = wh.y, sx = sxy.x, sy = sxy.y;
            if (this.group) {
                w = w * this.group.scaleX;
                h = h * this.group.scaleY;
            }
            ctx.strokeRect(~~(-(w / 2) - padding - strokeWidth / 2 * sx) - .5, ~~(-(h / 2) - padding - strokeWidth / 2 * sy) - .5, ~~(w + padding2 + strokeWidth * sx) + 1, ~~(h + padding2 + strokeWidth * sy) + 1);
            if (this.hasRotatingPoint && this.isControlVisible("mtr") && !this.get("lockRotation") && this.hasControls) {
                var rotateHeight = (this.flipY ? h + strokeWidth * sx + padding * 2 : -h - strokeWidth * sy - padding * 2) / 2;
                ctx.beginPath();
        ...
        drawControls: function(ctx) {
            ...
            var size = this.cornerSize, size2 = size / 2, strokeWidth2 = ~~(this.strokeWidth / 2), wh = fabric.util.transformPoint(new fabric.Point(this.getWidth(), this.getHeight()), this.getViewportTransform(), true), width = wh.x, height = wh.y, left = -(width / 2), top = -(height / 2), padding = this.padding, scaleOffset = size2, scaleOffsetSize = size2 - size, methodName = this.transparentCorners ? "strokeRect" : "fillRect";
            ctx.save();
            ctx.lineWidth = 1;
            ctx.globalAlpha = this.isMoving ? this.borderOpacityWhenMoving : 1;
            ctx.strokeStyle = ctx.fillStyle = this.cornerColor;
            this._drawControl("tl", ctx, methodName, left - scaleOffset - strokeWidth2 - padding, top - scaleOffset - strokeWidth2 - padding);
            this._drawControl("tr", ctx, methodName, left + width - scaleOffset + strokeWidth2 + padding, top - scaleOffset - strokeWidth2 - padding);
            this._drawControl("bl", ctx, methodName, left - scaleOffset - strokeWidth2 - padding, top + height + scaleOffsetSize + strokeWidth2 + padding);
            this._drawControl("br", ctx, methodName, left + width + scaleOffsetSize + strokeWidth2 + padding, top + height + scaleOffsetSize + strokeWidth2 + padding);
            if (!this.get("lockUniScaling")) {
                this._drawControl("mt", ctx, methodName, left + width / 2 - scaleOffset, top - scaleOffset - strokeWidth2 - padding);
                this._drawControl("mb", ctx, methodName, left + width / 2 - scaleOffset, top + height + scaleOffsetSize + strokeWidth2 + padding);
                this._drawControl("mr", ctx, methodName, left + width + scaleOffsetSize + strokeWidth2 + padding, top + height / 2 - scaleOffset);
                this._drawControl("ml", ctx, methodName, left - scaleOffset - strokeWidth2 - padding, top + height / 2 - scaleOffset);
            }
            if (this.hasRotatingPoint) {
                this._drawControl("mtr", ctx, methodName, left + width / 2 - scaleOffset, this.flipY ? top + height + this.rotatingPointOffset - this.cornerSize / 2 + strokeWidth2 + padding : top - this.rotatingPointOffset - this.cornerSize / 2 - strokeWidth2 - padding);
            }
            ctx.restore();
            return this;
        },
...
(function(global) {
    "use strict";
    var fabric = global.fabric || (global.fabric = {}), extend = fabric.util.object.extend, min = fabric.util.array.min, max = fabric.util.array.max, invoke = fabric.util.array.invoke, degreesToRadians = fabric.util.degreesToRadians;
    if (fabric.Group) {
        return;
    }
    var _lockProperties = {
