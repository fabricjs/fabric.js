...
this.sourcePath;delete a.sourcePath;return a},complexity:function(){return this.path.length},_parsePath:function(){for(var a=[],c,e,n=0,q=this.path.length;n<q;n++){c=this.path[n];e=c.slice(1).trim().replace(/(\d)-/g,"$1###-").split(/\s|,|###/);a.push([c.charAt(0)].concat(e.map(parseFloat)))}return a},_parseDimensions:function(){var a=[],c=[],e,n,q=false,s,x;this.path.forEach(function(v,E){if(v[0]!=="H")e=E===0?m(v):m(this.path[E-1]);if(v[0]!=="V")n=E===0?p(v):p(this.path[E-1]);if(v[0]===v[0].toLowerCase())q=
true;s=q?e+m(v):v[0]==="V"?e:m(v);x=q?n+p(v):v[0]==="H"?n:p(v);var C=parseInt(s,10);isNaN(C)||a.push(C);C=parseInt(x,10);isNaN(C)||c.push(C)},this);var y=h(a),A=h(c),u=deltaY=0;y={top:A-deltaY,left:y-u,bottom:o(c)-deltaY,right:o(a)-u};y.width=y.right-y.left;y.height=y.bottom-y.top;return y}});f.Path.fromObject=function(a){return new f.Path(a.path,a)};var g=f.Path.ATTRIBUTE_NAMES="d fill fill-opacity fill-rule stroke stroke-width transform".split(" ");f.Path.fromElement=function(a,c){var e=f.parseAttributes(a,
g),n=e.d;delete e.d;return new f.Path(n,l(e,c))}}else f.warn("fabric.Path requires fabric.Object")})();
