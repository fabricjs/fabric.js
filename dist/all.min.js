...
return this},forEachObject:function(b,a){for(var c=this.getObjects(),h=c.length;h--;)b.call(a,c[h],h,c);return this},_setOpacityIfSame:function(){var b=this.getObjects(),a=b[0]?b[0].get("opacity"):1;if(b.every(function(c){return c.get("opacity")===a}))this.opacity=a},_calcBounds:function(){var b=[],a=[],c,h;h=0;for(var n=this.objects.length;h<n;++h){c=this.objects[h];c.setCoords();for(var q in c.oCoords){b.push(c.oCoords[q].x);a.push(c.oCoords[q].y)}}c=d(b);h=m(b);b=d(a);a=m(a);h=h-c;a=a-b;this.width=
h;this.height=a;this.left=c+h/2;this.top=b+a/2},containsPoint:function(b){var a=this.get("width")/2,c=this.get("height")/2,h=this.get("left"),n=this.get("top");return h-a<b.x&&h+a>b.x&&n-c<b.y&&n+c>b.y},toGrayscale:function(){for(var b=this.objects.length;b--;)this.objects[b].toGrayscale()}});g.Group.fromObject=function(b){return new g.Group(b.objects,b)}}})();
