(function() {
  ...
  QUnit.test('triggerObjectsEvents', function (assert) {
    var addedWasCalled = false, removedWasCalled = false;
    ...
    canvas.add(group);
    canvas.remove(group);
    assert.equal(true, addedWasCalled && removedWasCalled, 'added & removed were called on object.');
  });

  QUnit.test('triggerObjectsEvents from JSON', function (assert) {
    var group = new fabric.Group([]);
    assert.deepEqual(group.triggerObjectsEvents, [], 'no events should be triggered on object.');
    group.set({ triggerObjectsEvents: ['added', 'removed'] });
    assert.deepEqual(group.triggerObjectsEvents, ['added', 'removed'], 'added & removed should be triggered on object.');
    fabric.Group.fromObject(group.toJSON(), function (newGroup) {
      assert.deepEqual(newGroup.triggerObjectsEvents, ['added', 'removed'], 'added & removed should be triggered on object.');
    });
  });
