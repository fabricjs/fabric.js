(function(){
  ...
  QUnit.test('toCanvasElement does not modify controlCoords on zoomed canvas', function(assert) {
    var cObj = new fabric.Rect({
      width: 100, height: 100, fill: 'red', strokeWidth: 0
    });
    canvas.setZoom(2);
    ...
    var originalaCoords = cObj.ownCoords;
    cObj.toCanvasElement();
    assert.deepEqual(cObj.controlCoords, originaloCoords, 'cObj did not get object coords changed');
    assert.deepEqual(cObj.ownCoords, originalaCoords, 'cObj did not get absolute coords changed');
  });
