(function() {
  ...
  QUnit.test('_beforeTransform', function (assert) {
    ...
    for (var i = 0; i < corners.length; i++) {
      var co = rect.controlCoords[corners[i]].corner;
      var e = {
    ...
    for (var i = 0; i < corners.length; i++) {
      var c = corners[i];
      var co = rect.controlCoords[c].corner;
      var e = {
        clientX: canvasOffset.left + (co.tl.x + co.tr.x) / 2,
        clientY: canvasOffset.top + (co.tl.y + co.bl.y) / 2,
        which: 1,
  ...
  QUnit.test('_setCursorFromEvent ', function(assert) {
    ...
    Object.entries(target.controlCoords).forEach(([corner, coords]) => {
    ...
    target.lockScalingX = false;
    const e = { clientX: target.controlCoords.mtr.x, clientY: target.controlCoords.mtr.y, [key]: false, target: canvas.upperCanvasEl };
    canvas._setCursorFromEvent(e, target);
