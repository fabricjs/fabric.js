(function() {
  ...
  QUnit.assert.correctShadowCoords = function (object, offsetX, offsetY, blur, vpt) {
    ...
    var control = object.getCoords(false, true).map((point, i) => {
      var rad = fabric.util.degreesToRadians(fabric.util.qrDecompose(object.calcTransformMatrix()).angle);
      var r = i * quarter + rad + 1;
      return new fabric.Point(
        point.x + offsetX - blur * Math.sign(Math.cos(r)),
        point.y + offsetY - blur * Math.sign(Math.sin(r))
      );
    });
    var coords = object.calcShadowCoords(false, true);
    ...
    //  abs
    control = object.getCoords(true, true).map((point, i) => {
      var rad = fabric.util.degreesToRadians(fabric.util.qrDecompose(object.calcTransformMatrix()).angle);
      var r = i * quarter + rad + 1;
      return new fabric.Point(
        point.x + offsetX - blur * Math.sign(Math.cos(r)),
    ...
    coords = object.calcShadowCoords(true, true);
    this.deepEqual(coords[0], control[0], 'abs tl');
    this.deepEqual(coords[1], control[1], 'abs tr');
    this.deepEqual(coords[2], control[2], 'abs br');
    this.deepEqual(coords[3], control[3], 'abs bl');
  }

  QUnit.test('calcShadowCoords in zoomed canvas coordinate.', function (assert) {
    var cObj = new fabric.Object({ width: 10, height: 15, strokeWidth: 2, left: 40, top: 30 });
  ...
  QUnit.test('calcShadowCoords with accute angle', function (assert) {
    var cObj = new fabric.Object({ width: 10, height: 15, strokeWidth: 2, left: 40, top: 30, angle: 120 });
    assert.correctShadowCoords(cObj, 40, 30, 1, [2, 0, 0, 2, 35, 35]);
  });
