(function() {
  ...
  QUnit.test('getFontCache works with fontWeight and fontCache numbers', function(assert) {
    var text = new fabric.Text('xxx', { fontWeight: 400, fontCache: 15 });
    text.initDimensions();
    var cache = fabric.charWidthsCache[text.fontFamily.toLowerCase()];
    var cacheProp = text.fontStyle + '_400_15';
    assert.equal(cacheProp in cache, true, 'fontWeight and fontCache is converted to string');
  });
  ...
  QUnit.test('_measureChar handles 0 width chars', function(assert) {
    ...
    assert.equal(fabric.charWidthsCache[text.fontFamily.toLowerCase()].normal_normal_15[zwc], 0, 'zwc is a 0 width char');
