(function() {
  ...
  QUnit.test("setShadow", function(assert) {
    ...
    assert.equal(
      text.setShadow("10px 8px 2px red"),
      text,
      "should be chainable"
    );
  ...
  QUnit.test("text styleHas", function(assert) {
    ...
    assert.equal(
      text.styleHas("stroke"),
      false,
      "the text style has no stroke"
    );
  ...
  QUnit.test(
    "getFontCache works with fontWeight and fontSize numbers",
    function(assert) {
      ...
      assert.equal(
        cacheProp in cache,
        true,
        "fontWeight and fontSize is converted to string"
      );
    }
  );
  ...
  QUnit.test("_measureChar handles 0 width chars", function(assert) {
    ...
    assert.equal(
      fabric.charWidthsCache[text.fontFamily.toLowerCase()].normal_normal_15[
        zwc
      ],
      0,
