(function() {
  ...
  test('viewportCenterObjectV', function() {
    ...
    equal(rect.getCenterPoint().y, canvas.width / 2 * canvas.getZoom(), 'object\'s "top" property should correspond to canvas element\'s center when canvas is not transformed');
    equal(rect.getCenterPoint().x, oldX, 'x position did not change');
    canvas.setZoom(2);
    canvas.viewportCenterObjectV(rect);
    equal(rect.getCenterPoint().y, canvas.width / (2 * canvas.getZoom()), 'object\'s "top" property should correspond to viewport center');
    equal(rect.getCenterPoint().x, oldX, 'x position did not change');
    canvas.absolutePan(pan, pan);
    canvas.viewportCenterObjectV(rect);
    equal(rect.getCenterPoint().y, (canvas.width / (2 * canvas.getZoom()) ) + pan, 'object\'s "top" property should correspond to viewport center');
    equal(rect.getCenterPoint().x, oldX, 'x position did not change');
  });
