...
export function useCanvas(init, saveState = true) {
  ...
  const _init = useRef(DEV_MODE ? init?.toString() : undefined);

  const setRef = useCallback(ref => {
    //@ts-ignore
    elementRef.current = ref; // save state
    ...
    fc.current?.dispose(); // set/clear ref
    ...
    fc.current = canvas; // invoke callback

    init && init(canvas); // restore state
