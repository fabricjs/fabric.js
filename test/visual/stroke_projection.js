...
(function() {
  ...
  const casesToTest = {
    noMiterAfterMiterLimit2: [{x: 0, y: 0}, {x: 10, y: 30}, {x: 43, y: 0}],
    acuteAngle: [{x: 3, y: -10}, {x: 18, y: 16}, {x: 123, y: -26}],
    obtuseAngle: [ {x: 16, y: -29}, {x: 187, y: 50}, {x: 123, y: -26}],
    rightAngle: [{x:0, y:0}, {x:0, y:-30}, {x:30, y:-30}],
    straightAngle: [{x: 1, y:1}, {x: 6, y:6}, {x:36, y:36}],
    twoPointsClose: [{x: 0, y: 0}, {x: 2, y: -0.98}, {x: 100, y: 0}],
    convex: [{x: 20, y: 4}, {x: 20, y: 27}, {x: 60, y: 27}, {x: 60, y: 4}, {x: 40, y: -10}],
    concave: [{x: 23.23027292144134, y: -20.710253258520993}, {x: 20, y: 27}, {x: 68, y: 37}, {x: 90, y: -4}, {x: 48, y: 12}],
    complex: [{x: 20, y: 4}, {x: 10, y: 27}, {x: 60, y: 27}, {x: 70, y: 4}, {x: 40, y: 37}],
    star: [{x: 0, y: 0}, {x: 72, y: 36}, {x: 36, y: -18}, {x: 0, y: 36}, {x: 72, y: 0}],
    plus: [
      {x: 0, y: 0},
      {x: 0, y: 20},
      {x: 20, y: 20},
  ...
  for (let [caseName, casePoints] of Object.entries(casesToTest)) {
    [fabric.Polyline, fabric.Polygon].forEach((builder) => {
      const type = builder.prototype.type,
        ...
        strokes = isPolygon ? ['miter', 'round', 'bevel'] : ['butt', 'square', 'round'];
      strokes.forEach((strokeLineType) => {
        [true, false].forEach((strokeUniform) => {
          [[0, 30], [20, 0], [25, 35]].forEach(([skewX, skewY]) => {
              if (strokeLineType === 'round' && skewX !== 0 & skewY !== 0) return; // TODO: remove this line when fix strokeLineJoins equals `round` with `skewX`and `skewY` applied at sametime
              tests.push({
                test: `${caseName} of type ${type} with ${isPolygon ? 'strokeLineJoin' : 'strokeLineCap'}=${strokeLineType}, strokeUniform=${strokeUniform}, skewX=${skewX} and skewY=${skewY} values`,
                ...
                golden: `stroke-projection/${isPolygon ? 'strokeLineJoin' : 'strokeLineCap'}/${strokeLineType}/${caseName}-${strokeUniform ? 'uniform-' : ''}${type}-${skewX}skewX-${skewY}skewY.png`,
                percentage: 0.001,
                width: 600,
                height: 900,
                fabricClass: 'Canvas',
      ...
      [true, false].forEach((strokeUniform) => {
        [[0, 30], [20, 0], [25, 35]].forEach(([skewX, skewY]) => {
            tests.push({
              ...
              golden: `stroke-projection/strokeLineJoin/miter-limit/${strokeUniform ? 'uniform-' : ''}${caseName}-${strokeMiterLimit}miterLimit-${skewX}skewX-${skewY}skewY.png`,
              percentage: 0.001,
              width: 600,
              height: 900,
              fabricClass: 'Canvas'
